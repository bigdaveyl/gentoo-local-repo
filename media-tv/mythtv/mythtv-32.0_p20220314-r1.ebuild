...
if [[ $(ver_cut 3) == "p" ]] ; then
	MY_COMMIT="ae60e8302fab769dd0aacb091d195b1cef08ad9c"
...
IUSE="alsa aom asi autostart bluray cdda cdr cec cpu_flags_ppc_altivec dav1d
	  debug drm dvd dvb egl exiv2 fftw fontconfig freetype jack
	  java +lame lcd libass lirc lto nvdec +opengl oss perl
	  pulseaudio python raw sdl systemd taglib udf vaapi vdpau
	  vpx vulkan +wrapper x264 x265 +xml xmltv
	  +xvid +X zeroconf
...
src_configure() {
	...
	myconf+=(
		...
		$(use_enable ieee1394 libiec61883)
	...
	myconf+=(
		...
		$(use_enable fontconfig libfontconfig)
		$(use_enable freetype libfreetype)
		$(use_enable drm)
	)
	...
	myconf+=(
		...
		$(use_enable exiv2 system_libexiv2)
		$(use_enable udf system_libudfread)
		$(use_enable taglib)
		$(use_enable aom libaom)
		$(use_enable dav1d libdav1d)
	)
	...
	myconf+=($(use_enable bluray system_libbluray))
